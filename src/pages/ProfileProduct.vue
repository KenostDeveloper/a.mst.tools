	...
	<div class="product_form">
		<div class="dart-row dart-align-items-center">
			<div class="d-col-lg-6" v-if="product.product_id > 0">
				...
				<p class="info-text">
					Чтобы привязать товар из 1С, найдите этот товар в нашем каталоге. Если товара в
					каталоге нет, оформите запрос на создание карточки товара.
				</p>
				<div class="form_input_group">
					...
					>
						<template #item="slotProps">
							<img
								...
								:src="'http://mst.tools/' + slotProps.item.image"
							/>
					...
					<a
						href="#"
						class="request_alert"
						@click.prevent="showRequestModal = true"
						v-if="getrequests.total < 1 && product.product_id == 0"
						>Запросить создание карточки товара</a
					>
				</div>
				<div class="linked_product mt-4">
					...
					<div class="text">
						...
						<span class="desc"
							><b class="lb">Артикул:</b> {{ product.product_article }}</span
						>
						<span class="desc"
							><b class="lb">Артикул производителя:</b>
							{{ product.vendor_article }}</span
						>
					</div>
				</div>
			</div>
		...
		<custom-modal v-model="showRequestModal" @cancel="cancelRequest" class="plan-modal">
			...
			<form @submit.prevent="onSubmitRequest">
				...
				<button
					...
					type="submit"
					:class="{ 'dart-btn-loading': loadingRequest }"
...
export default {
					...
					return Axios("/rest/front_deleteobject", {
						...
						.then((response) => {
							this.$toast.add({
								severity: "info",
								summary: "Удаление",
								detail: "Заявка " + data.name + " успешно удалена",
								life: 3000,
							});
						...
						.catch((error) => {
							if (error.response.status === 403) {
								this.$toast.add({
									...
									detail: "Вы будете перенаправлены на страницу авторизации",
									life: 3000,
								});
	...
	computed: {
		...mapGetters(["product", "getrequests"]),
	},
};
</script>
