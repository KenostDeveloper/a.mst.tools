  ...
  <form @submit.prevent="formSubmit">
    <div class="profile-content__title sticky-element">
      ...
      <div class="buttons_container">
        <RouterLink :to="{ name: 'b2c', params: { id: $route.params.id } }"
          class="dart-btn dart-btn-secondary btn-padding">Отменить</RouterLink>
        <button type="submit" class="dart-btn dart-btn-primary btn-padding" :class="{ 'dart-btn-loading': loading }"
          :disabled="loading">Сохранить</button>
      </div>
    </div>
    <div>
      <div>
        <p class="kgraytext m-0" v-if="this.status">Статус: <span
            :class="{ 'status-moderation': this.status === 'Модерация', 'status-fatal': this.status === 'Отказ', 'status-planned': this.status === 'Запланирована', 'status-active': this.status === 'Активна' }">{{ this.status }}</span>
        </p>
        <p class="kgraytext" v-if="this.moderator_comment != '' && this.moderator_comment">Комментарий от модератора: {{
          this.moderator_comment }}</p>
      </div>
      ...
      <div class="dart-form-group mt-2 mb-4">
        ...
        <input v-model="form.name" type="text" name="name" placeholder="Укажите название акции"
          class="dart-form-control" :class="{ 'kenost-error': this.validation.name.error }">
      ...
      <div class="dart-form-group mt-2 mb-4">
        ...
        <Dropdown @change="updateProducts" v-model="this.form.store_id" :options="this.stores" optionLabel="label"
          optionValue="value" placeholder="Выберите склад" />
      </div>
      ...
      <div v-if="this.form.store_id">
        <div class="dart-form-group mb-0">
          ...
          <div class="flex align-items-center gap-1">
            <Checkbox v-model="this.create_page_action" inputId="create-page-action" name="create_page_action"
              value="true" />
        ...
        <div class="dart-form-group kenost-action-page pt-3" v-if="this.create_page_action.length != 0">
          <div class="upload-banner">
            <div class="upload-banner__text">
              ...
              <span>Загрузить изображение нужно размером не менее 1108х332, соблюдая пропорции. Чтобы не потерялось
                качество, желательно загружать изображение в два раза больше указанного размера.</span>
            </div>
            ...
            <FileUpload class="kenost-upload-button" mode="basic" name="banner[]"
              :url="'/rest/file_upload.php?banner=max'" accept="image/*" :maxFileSize="2000000" @upload="onUpload"
              :auto="true" chooseLabel="Загрузить" />
          </div>
        ...
        <div class="dart-form-group kenost-action-page pt-3" v-if="this.create_page_action.length != 0">
          <div class="upload-banner">
            <div class="upload-banner__text">
              ...
              <span>Загрузить изображение нужно размером не менее 324х161, соблюдая пропорции. Чтобы не потерялось
                качество, желательно загружать изображение в три раза больше указанного размера.</span>
            </div>
            <FileUpload class="kenost-upload-button" mode="basic" name="banner_small[]"
              :url="'/rest/file_upload.php?banner=min'" accept="image/*" :maxFileSize="2000000" @upload="onUpload"
              :auto="true" chooseLabel="Загрузить" />
          </div>
          <div class="upload-banner__image">
        ...
        <div class="dart-form-group kenost-action-page pt-3" v-if="this.create_page_action.length != 0">
          <div class="upload-banner">
            <div class="upload-banner__text">
              ...
              <span>Загрузить изображение нужно размером не менее 459х459, соблюдая пропорции. Чтобы не потерялось
                качество, желательно загружать изображение в три раза больше указанного размера.</span>
            </div>
            <FileUpload class="kenost-upload-button" mode="basic" name="small[]"
              :url="'/rest/file_upload.php?banner=small'" accept="image/*" :maxFileSize="2000000" @upload="onUpload"
              :auto="true" chooseLabel="Загрузить" />
          </div>
          <div class="upload-banner__image">
        ...
        <div class="dart-form-group kenost-action-page pt-3" v-if="this.create_page_action.length != 0">
          <div class="rules-container">
            <div class="rules-container__text">
              ...
              <a target="_blank" :href="files?.file?.original_href" class="kenost-add-file"
                v-if="files?.file?.original_href">
                <!-- <img src="../../../public/img/files/pdf.png" alt=""> -->
                <p>{{ files?.file?.name ? files?.file?.name : "Файл загружен!" }}</p>
              </a>
            </div>
            <FileUpload class="kenost-upload-button" mode="basic" name="icon[]"
              :url="'/rest/file_upload.php?banner=file'" accept="application/pdf" :maxFileSize="20000000"
        ...
        <div class="dart-form-group kenost-action-page pt-3" v-if="this.create_page_action.length != 0">
          ...
          <Dropdown v-model="this.geo_action" :options="this.geo" optionLabel="name" placeholder="География показа"
            class="w-full md:w-14rem" />
        </div>

        <div class="dart-form-group kenost-action-page pt-3" v-if="this.create_page_action.length != 0">
          <span class="ktitle">Позиция в карусели</span>
          <InputNumber v-model="this.position" inputId="horizontal-buttons" :step="1" min="0" max="100"
            incrementButtonIcon="pi pi-plus" decrementButtonIcon="pi pi-minus" />
        </div>

        <div class="dart-form-group mb-4 mt-4">
          <span class="ktitle">Условия</span>
          <input v-model="form.conditions" type="text" name="conditions" placeholder="Укажите условия акции"
        ...
        <div class="dart-form-group mb-4">
          ...
          <Editor api-key="ctqgmxpl4dimvsnrt6lnhbyk2xb7eyrhvbbh9lch2kltngkh"
            language_url='../../../src/locales/tiny/ru.js' :language_load=true v-model="this.form.description"
        ...
        <div class="dart-form-group mb-4">
          ...
          <Calendar v-model="form.dates" selectionMode="range" placeholder="Выберите даты" :manualInput="false" showIcon
            :class="{ 'kenost-error': this.validation.dates.error }" />
        ...
        <div class="dart-form-group mb-4">
          ...
          <MultiSelect :class="{ 'kenost-error': this.validation.region.error }" v-if="this.region_all.length == 0" filter
            v-model="this.select_regions" display="chip" :options="this.regions_all" optionLabel="name"
            placeholder="Выберите регионы" class="w-full md:w-20rem kenost-multiselect" />
        ...
        <div v-if="this.form.addProductType == '2'" class="dart-form-group mb-4">
          <DropZone v-if="!this.upload_product" class="kenost-dropzone" :maxFiles="Number(1)"
            ...
            :acceptedFiles="['xls', 'xlsx']" :parallelUpload="1" @sending="parseFile" v-bind="args">
          ...
          <a :href="site_url_prefix + '/assets/files/files/examples/ExampleLoadingProductsB2C.xlsx'"
        ...
        <div class="dart-form-group picker-wrap">
          ...
          <div class="PickList">
            <div class="PickList__product d-col-5">
              ...
              <div class="PickList__filters">
                ...
                <div class="dart-form-group">
                  <TreeSelect v-model="this.filter.category" :options="this.opt_catalog_tree" selectionMode="checkbox"
              ...
              <div class="PickList__products">
                ...
                <paginate :page-count="pagesCount" :click-handler="pagClickCallback" :prev-text="'Пред'"
                  :next-text="'След'" :container-class="'pagination justify-content-center'" :initialPage="this.page"
                  :forcePage="this.page">
                </paginate>
              </div>
            </div>
            ...
            <div class="PickList__buttons">
              ...
              <div class="PickList__select mt-2" @click="setAllProducts(false)"><i class="pi pi-angle-double-left"></i>
              </div>
            </div>
        ...
        <div class="table-kenost mt-4 table-b2c" :class="{ loading: this.table_products_loading }">
          ...
          <div class="table-kenost__filters">
            <div class="table-kenost__filters-left">
              ...
              <div class="dart-form-group">
                <TreeSelect label="name" v-model="this.filter_table.category" :options="this.opt_catalog_tree"
                  selectionMode="checkbox" placeholder="Выберите категорию" class="w-full" @change="setFilter" />
              </div>
            </div>
            ...
            <tbody v-for="item in this.selected_visible" :key="item.id">
              <tr>
                <td class="table-kenost__checkbox">
                  <Checkbox @change="kenostTableCheckedAllCheck" v-model="this.kenost_table" inputId="kenost_table"
                    :value="item.id" />
                </td>
          ...
          <paginate :page-count="pagesCountSelect" :click-handler="pagClickCallbackSelect" :prev-text="'Пред'"
            :next-text="'След'" :container-class="'pagination justify-content-center'" :initialPage="this.page_selected"
          ...
          <div class="table-kenost-help">
            ...
            <div class="flex align-items-center gap-1">
              <Checkbox @change="globalTable" v-model="this.form.global_kenost_table" inputId="global_kenost_table-1"
                name="global_kenost_table-1" value="true" />
        ...
        <div class="kenost-all-table-activity"
          ...
          <div class="kenost-wiget" v-if="this.kenostActivityAll.type.key == 0 || this.kenostActivityAll.type.key == 1">
            <p>Тип цен</p>
            <Dropdown v-model="this.kenostActivityAll.typePrice" :options="this.typePrice" optionLabel="name"
              placeholder="Тип цен" class="w-full md:w-14rem" />
          </div>
          <div class="kenost-wiget" v-if="this.kenostActivityAll.type.key == 0">
            <p>Значение</p>
            <InputNumber v-model="this.kenostActivityAll.value" inputId="horizontal-buttons" :step="1" min="0"
              incrementButtonIcon="pi pi-plus" decrementButtonIcon="pi pi-minus" />
          </div>
          <div class="kenost-wiget" v-if="this.kenostActivityAll.type.key == 0">
            <p>&nbsp;</p>
          ...
          <div class="kenost-wiget" v-if="this.kenostActivityAll.type.key == 3">
            <p>Значение</p>
            <InputNumber v-model="this.kenostActivityAll.multiplicity" inputId="horizontal-buttons" :step="1" min="1"
              incrementButtonIcon="pi pi-plus" decrementButtonIcon="pi pi-minus" />
          </div>

          <div
            v-if="this.kenostActivityAll.type.key == 0 || this.kenostActivityAll.type.key == 1 || this.kenostActivityAll.type.key == 2 || this.kenostActivityAll.type.key == 3"
            @click="massActionTable" class="dart-btn dart-btn-primary mt-3"><i class="pi pi-check"></i></div>
        </div>
      </div>
    </div>
  </form>
    ...
    <div class="kenost-modal-price">
      ...
      <div v-if="this.modals.price_step == 1" class="two-colums mt-3">
        <div class="kenost-wiget">
          <p>Тип цены</p>
          <Dropdown
            @change="setDiscountFormul(this.selected_data[this.modals.product_id].typeFormul, this.saleValue, this.selected_data[this.modals.product_id].typePrice)"
            v-model="this.selected_data[this.modals.product_id].typePrice"
            :options="this.selected[this.modals.product_id].prices" optionLabel="name" class="w-full md:w-14rem" />
        </div>
        <div class="kenost-wiget-two">
          ...
          <div class="kenost-wiget">
            ...
            <Dropdown
              @change="setDiscountFormul(this.selected_data[this.modals.product_id].typeFormul, this.saleValue, this.selected_data[this.modals.product_id].typePrice)"
              v-model="this.selected_data[this.modals.product_id].typeFormul" :options="this.typeFormul"
      ...
      <div v-if="this.modals.price_step == 3" class="two-colums mt-3">
        <div class="kenost-wiget">
          ...
          <InputNumber v-model="this.selected_data[this.modals.product_id].discountInterest"
            inputId="horizontal-buttons" :step="1" min="0" max="100" suffix=" %"
        ...
        <div class="kenost-wiget">
          <p>Скидка в ₽</p>
          <InputNumber v-model="selected_data[this.modals.product_id].discountInRubles" inputId="horizontal-buttons"
            :step="1" min="0" :max="selected[this.modals.product_id].price" mode="currency" currency="RUB"
        ...
        <div class="kenost-wiget">
          <p>Цена со скидкой</p>
          <InputNumber v-model="selected_data[this.modals.product_id].finalPrice" inputId="horizontal-buttons" :step="1"
            :max="selected[this.modals.product_id].price" mode="currency" currency="RUB" min="0"
