    ...
    <div class="table-actions__content">
        <div class="table-actions__modal" :class="{'pt-3': (!action?.image.image || action?.type == 3 || !action?.image.thumb_medium) && !action.image}">
            ...
            <div class="kenost-action-modal">
                <div class="kenost-action-modal__item" v-for="(tag, index) in action.tags.slice(0, !(action.tags.length % 2)?  action.tags.length - Math.floor(action.tags.length / 2) + 1:action.tags.length - Math.floor(action.tags.length / 2))" v-bind:key="tag.id">
